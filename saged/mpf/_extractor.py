...
from saged._utility import ignore_future_warnings
...
from saged._utility import check_benchmark, clean_sentences_and_join
...
class FeatureExtractor:
    ...
    @ignore_future_warnings
    def anchor_distance(self, anchor_text, distance_function='cosine'):
        """
        Calculate embedding distances between target texts and an anchor text.
        
        Args:
            anchor_text (str): The anchor text to compare against
            distance_function (str): The type of distance function to use ('cosine', 'l1', 'l2')
            
        Returns:
            pd.DataFrame: DataFrame with added distance columns for each generation
        """
        if not self._model:
            print('Using default embedding model: all-MiniLM-L6-v2')
        ...
        def _compute_distance(text, anchor_vector, distance_function=distance_function):
            # Get embedding for the text
            text_vector = self.embedding_model.encode([text])[0]
            
            # Compute the distance between text and anchor vector
            if distance_function == 'cosine':
                cosine_dist = 1 - _cosine_similarity(text_vector, anchor_vector)
                return cosine_dist
            elif distance_function == 'l1':
                l1_dist = np.sum(np.abs(text_vector - anchor_vector))
                return l1_dist
            elif distance_function == 'l2':
                l2_dist = np.linalg.norm(text_vector - anchor_vector)
                return l2_dist
            else:
                raise ValueError("Invalid distance function specified. Choose from 'cosine', 'l1', 'l2'.")
                
        # Get embedding for the anchor text
        anchor_vector = self.embedding_model.encode([anchor_text])[0]
        ...
        for generation in self.generations:
            distance_column = f'{generation}_anchor_distance'
            df[distance_column] = df[generation].apply(
                lambda text: _compute_distance(text, anchor_vector, distance_function)
            )
            self.classification_features.append('anchor_distance')
            
        if self.calibration:
            df = self._baseline_calibration(['anchor_distance'])
    ...
    def llm_score(self, 
                 ...
                 save_interval=20,
                 save_path=None,
                 feature_name='llm_score'):
        """
        Score texts using an LLM based on a provided prompt template and generation function.
        
        Args:
            system_promptable_generation_function: Function that takes (prompt, system_prompt) and returns LLM response
            prompt_template (str): The prompt template to use for scoring. Should contain {text} placeholder.
            score_range (tuple): The range of scores to normalize to (min, max). Default is (0, 1).
            generation_name (str): Name for the generation column. Default is 'LLM'.
            save_interval (int): Number of rows to process before saving progress. Default is 20.
            save_path (str): Path to save progress. Default is None.
            feature_name (str): Name of the feature to be added to classification_features. Default is 'llm_score'.
            
        Returns:
            pd.DataFrame: DataFrame with added score columns for each generation
        """
        ...
        def _normalize_score(score, score_range):
            """Normalize score to the specified range"""
            min_score, max_score = score_range
            return min_score + (max_score - min_score) * score
        ...
        for i in tqdm(range(0, total_rows, save_interval), desc="Overall Progress"):
            ...
            current_slice = df.iloc[i:end_idx].copy()
            
            # Calculate scores for each generation
            for generation in self.generations:
                score_column = f'{generation}_{feature_name}'
                
                def get_score(row):
                    try:
                        # Format the prompt with the text
                        prompt = prompt_template.format(text=row[generation])
                        
                        # Get response from LLM using the provided generation function
                        response = system_promptable_generation_function(
                            prompt=prompt,
                            system_prompt="You are a scoring assistant. Provide only a numerical score."
                        )
                        
                        # Extract and normalize score
                        score = _extract_score(response)
                        return _normalize_score(score, score_range)
                    except Exception as e:
                        print(f"Error getting LLM score: {e}")
                        return 0.5  # Default score on error
                
                # Apply scoring with progress bar
                tqdm.pandas(desc="Batch Processing", leave=False)
                current_slice[score_column] = current_slice.progress_apply(get_score, axis=1)
                self.classification_features.append(feature_name)
            
            # Update the main dataframe with the processed slice
            df.iloc[i:end_idx] = current_slice
        ...
        if self.calibration:
            df = self._baseline_calibration([feature_name])
