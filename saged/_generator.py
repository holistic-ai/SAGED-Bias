...
Please synthesize these responses into a single coherent answer:"""
    ...
    def aggregated_pre_generate(self,
                              ...
                              system_promptable_generation_function,
                              num_generations=3,
                              ensemble_system_prompt="Carefully read the following multiple responses. Your task is to produce a single, concise, and factually accurate answer that fully integrates all original statements into a unified response. You are strictly prohibited from copying any sentences or phrases from the original responses, referring to or commenting on the responses in any way (such as \"Response A states...\" or \"All three responses...\"), summarizing them individually, omitting or altering any viewpoint, or adding new information, interpretations, or speculative language. Only rephrase and merge the original factual content into one coherent answer.",
                              generation_name='LLM',
        ...
        # Create a temporary column for tracking progress
        self.benchmark[generation_name] = None
        #subset = self.benchmark.sample(n=50, random_state=42)
        # Process each row
        for idx, row in tqdm(self.benchmark.iterrows(), total=len(self.benchmark), desc="Overall Progress"):
            concept = row['concept']
            if not concept:
