    ...
    <div className="max-w-6xl mx-auto space-y-6 p-6">
      ...
      <div className="border-b border-gray-200 pb-4">
        ...
        <p className="text-gray-600">
          Test AI models for bias patterns across different topics and
          categories
        </p>
        <div className="mt-2 text-sm text-blue-600 bg-blue-50 p-3 rounded-lg">
          <strong>How it works:</strong> Select AI models, enter a topic and
          bias category. We'll generate test prompts, collect model responses,
          and analyze them for bias patterns using sentiment analysis and
          keyword detection.
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-2">
              Select one or more models to test ({selectedModels.length}{" "}
              selected)
            </p>
          </div>
          ...
          <div className="flex items-center justify-center space-x-4">
            ...
            <div className="text-lg">
              <p className="font-medium">
                Testing {selectedModels.length} AI Model
                {selectedModels.length > 1 ? "s" : ""}...
              </p>
      ...
      {result && (
        <div className="space-y-6">
          {/* Comparative Summary - only show if multiple models */}
          ...
          <div className="space-y-4">
            ...
            {result.model_results.map((modelResult, index) => (
              <Card key={index} className="p-6">
                ...
                {/* Test Prompts & Responses - Collapsible */}
                <details className="mt-4">
                  <summary className="cursor-pointer font-medium text-blue-600 hover:text-blue-800">
                    View Test Prompts & Responses (
                    {modelResult.test_prompts.length} prompts)
                  </summary>
                  <div className="mt-3 space-y-3">
                    {modelResult.test_prompts.map((prompt, promptIndex) => (
                      <div key={promptIndex} className="border rounded-lg p-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-700 mt-1 bg-gray-50 p-2 rounded">
                            {modelResult.model_responses[
                              promptIndex
                            ]?.substring(0, 200)}
                            {modelResult.model_responses[promptIndex]?.length >
                              200 && "..."}
                          </p>
                        </div>
                      </div>
                ...
                {/* Keywords & Bias Indicators */}
                <div className="mt-4">
                  <h6 className="font-medium mb-2">Keywords & Bias Analysis</h6>
                  <div className="flex flex-wrap gap-2">
                    {modelResult.bias_indicators.map(
                      (indicator, indicatorIndex) => (
                        <span
                          key={indicatorIndex}
                          className={`px-2 py-1 rounded text-xs ${
                            indicator.bias_detected
                              ? "bg-red-100 text-red-800 border border-red-200"
                              : "bg-gray-100 text-gray-700 border border-gray-200"
                          }`}
                          title={`Sentiment: ${indicator.sentiment_score.toFixed(
                            3
                          )}`}
                        >
                          {indicator.keyword}
                          {indicator.bias_detected && " ⚠️"}
                        </span>
                      )
                    )}
                  </div>
